...
export const PageHeader = ({ eyebrow, title, description, actions }) => (
  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 sm:mb-8 pb-5 sm:pb-6 border-b border-[var(--bx-border)] gap-3">
    <div className="min-w-0">
      {eyebrow && (
        <div className="text-[10px] uppercase tracking-[0.14em] text-[var(--bx-text-2)] mb-2 font-semibold">
          {eyebrow}
        </div>
      )}
      <h1 className="bx-heading text-2xl lg:text-3xl tracking-tight text-[var(--bx-text)]">{title}</h1>
      {description && <p className="mt-2 text-sm text-[var(--bx-text-2)] max-w-2xl leading-relaxed">{description}</p>}
    </div>
...
export const KpiCard = ({ label, value, sub, accent }) => (
  <div className="bx-kpi" data-testid={`kpi-${label.toLowerCase().replace(/\s+/g, "-")}`}>
    <div className="text-[10px] uppercase tracking-[0.14em] text-[var(--bx-text-2)] mb-2 font-semibold">{label}</div>
    <div className={`bx-heading text-2xl tracking-tight text-[var(--bx-text)] ${accent || ""}`}>{value}</div>
    {sub && <div className="text-xs text-[var(--bx-text-2)] mt-1.5">{sub}</div>}
  </div>
);
...
export const Section = ({ title, children, action, className = "" }) => (
  <section className={`bg-[var(--bx-card)] border border-[var(--bx-border)] rounded-lg ${className}`}>
    <div className="px-4 sm:px-5 py-3.5 border-b border-[var(--bx-border)] flex items-center justify-between gap-2">
      <div className="text-[11px] uppercase tracking-[0.14em] text-[var(--bx-text)] font-semibold">
        {title}
      </div>
      {action}
    </div>
